...
def epsilon_greedy(Q,
    ...
    if eps_type == 'constant':
        ...
        if np.random.rand(1)> epsilon:
            maximum = Q[state][all_actions[0]]
            action = all_actions[0]
            for i in all_actions:
                if Q[state][i] > maximum:
                    maximum = Q[state][i]
                    action = i
        else:
        ...
        # It is recommended you use the np.random module
        schedlinear = ScheduleLinear(anneal_timesteps,epsilon_final,epsilon_initial)
        epsilon = schedlinear.value(current_total_steps)
        if np.random.rand(1) < epsilon:
            action = random.choice(all_actions)
        else:
            maximum = Q[state][all_actions[0]]
...
class PlayerControllerRL(PlayerController, FishesModelling):
    ...
    def player_loop(self):
        ...
        return
    def converged(self,database):
        maximum = max(database)
        new_d = database[-6:]
        return all(x == maximum for x in new_d)
    ...
    def q_learning(self):
        ...
        database = []
        converged = False
        while episode <= self.episode_max and (not converged or diff>self.threshold):
            s_current = init_pos
            R_total = 0
            steps = 0
            ...
            database.append(R_total)
            if episode >5:
                converged = self.converged(database)
            episode += 1
            end_episode = False
...
class ScheduleLinear(object):
    ...
    def value(self, t):
        ...
        diff_eps = self.final_p - self.initial_p
        epsilon = self.initial_p + diff_eps * (t/self.schedule_timesteps)
        return epsilon
        # ADD YOUR CODE SNIPPET BETWEEN EX 3.2